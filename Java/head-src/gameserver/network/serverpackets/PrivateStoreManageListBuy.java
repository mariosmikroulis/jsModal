...
public class PrivateStoreManageListBuy extends L2GameServerPacket
{
	...
	{
		_activeChar = player;
		 L2ItemInstance[] _ifPlayerIsNew;
		 TradeItem[] _ifPlayerCanBuy;
		...
		if(Config.TRADE_DROP_PICK_BUY_SELL_ENABLED && Config.BUY_TIME_DISABLE!=0 && player.getOnlineTime()<Config.BUY_TIME_DISABLE)
		{
			_ifPlayerIsNew=null;
			_ifPlayerCanBuy=null;
			player.sendMessage("You are in Buy Protection Mode for the next " + Config.BUY_TIME_TEXT + " since you have created this char. We apologise for that but it is kind of our own protection. If you believe that you are longer than that time, please restart your character now!");
		}
		else
		{
			_ifPlayerIsNew=_activeChar.getInventory().getUniqueItems(false, true, true);
			_ifPlayerCanBuy=_activeChar.getBuyList().getItems();
		}
		_itemList = _ifPlayerIsNew;
		_buyList = _ifPlayerCanBuy;
	}
