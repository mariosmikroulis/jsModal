...
public class AdminBan implements IAdminCommandHandler
{
	...
	public boolean useAdminCommand(final String command, final L2PcInstance activeChar)
	{
		...
		else if (command.startsWith("admin_ban_acc"))
		{
			...
			{
				if(Config.ENABLEDCONNECTIONFORUM)
				{
					
					try
					{
						//URL myURL = new URL(Config.BANFILEURL + "?action=unbanAcc&user="+ player);
						String charset= "UTF-8";
			       		String query = String.format("action=%s&user=%s", 
			       	    URLEncoder.encode("banAcc", charset), 
			       	    URLEncoder.encode(player, charset));
			       		URLConnection connection = new URL(Config.BANFILEURL + "?" + query).openConnection();
					...
					{   
						e.printStackTrace();
						System.out.println("2. Ban: unable to connect developer site!");
					}
				...
				if(Config.ENABLEDCONNECTIONFORUM)
				{
					try
					{
						...
						String charset= "UTF-8";
			       		String query = String.format("action=%s&user=%s", 
			       	    URLEncoder.encode("banAcc", charset), 
			       	    URLEncoder.encode(targetPlayer.getAccountName(), charset));
			       		URLConnection connection = new URL(Config.BANFILEURL + "?" + query).openConnection();
			       		connection.setRequestProperty("Accept-Charset", charset);
			       		InputStream response = connection.getInputStream();
					}
					...
					{   
						e.printStackTrace();
						System.out.println("1. Ban: unable to connect developer site!");
					}
					catch (final Exception e)
					{
						e.printStackTrace();
			...
			else if (targetPlayer == null)
			{
				if(Config.ENABLEDCONNECTIONFORUM)
				{
					try
					{
						...
						String charset= "UTF-8";
			       		String query = String.format("action=%s&user=%s", 
			       	    URLEncoder.encode("banIP", charset), 
			       	    URLEncoder.encode(player, charset));
			       		URLConnection connection = new URL(Config.BANFILEURL + "?" + query).openConnection();
			       		connection.setRequestProperty("Accept-Charset", charset);
			       		InputStream response = connection.getInputStream();
					...
					{   
						e.printStackTrace();
						System.out.println("BAN: I am unable to connect on the developer site!");
					}
					catch (final Exception e)
					{
						e.printStackTrace();
				...
				LoginServerThread.getInstance().sendAccessLevel(player, -100);
				activeChar.sendMessage("Ban IP request sent for account " + player);
				auditAction(command, activeChar, player);
			}
			else
			{
				...
				targetPlayer.setPunishLevel(L2PcInstance.PunishLevel.ACC, 0);
				activeChar.sendMessage("Account " + targetPlayer.getAccountName() + " banned. IP ban has been set.");
				auditAction(command, activeChar, targetPlayer.getAccountName());
			}
		}
			...
			else if (!player.equals(""))
			{
				if(Config.ENABLEDCONNECTIONFORUM)
				{
					...
					try
					{
						...
						String charset= "UTF-8";
			       		String query = String.format("action=%s&user=%s", 
			       	    URLEncoder.encode("unbanAcc", charset), 
			       	    URLEncoder.encode(player, charset));
			       		URLConnection connection = new URL(Config.BANFILEURL + "?" + query).openConnection();
			       		connection.setRequestProperty("Accept-Charset", charset);
			       		InputStream response = connection.getInputStream();
		...
		else if (command.startsWith("admin_unban_ip"))
		{
			...
			else if (!player.equals(""))
			{
				try
				{
					...
					String charset= "UTF-8";
		       		String query = String.format("action=%s&user=%s", 
		       	    URLEncoder.encode("unbanIP", charset), 
		       	    URLEncoder.encode(player, charset));
		       		URLConnection connection = new URL(Config.BANFILEURL + "?" + query).openConnection();
		       		connection.setRequestProperty("Accept-Charset", charset);
		       		InputStream response = connection.getInputStream();
				...
				activeChar.sendMessage("Unban IP request sent for account " + player+". Warning: This is only to unban IP but not the user name from the forum or the gameserver. In order to unban the account, please use the command \"//unban_acc " + player+"\".");
				auditAction(command, activeChar, player);
			}
			else
			{
				activeChar.sendMessage("Usage: //unban_ip <account_name>");
				return false;
			}
		}
