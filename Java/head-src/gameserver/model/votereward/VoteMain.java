...
public class VoteMain
{
	...
	{
		System.out.println("INFO  ===============================================================- [ Vote Reward ]");
		System.out.println("INFO  Vote Reward Per Person Started Successfully.");
	...
	public static String hopCd(L2PcInstance player)
	{
		...
		LOGGER.warn("Start testing...");
		long hopCdMs = 0;
		long voteDelay = 43200000L;
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			PreparedStatement statement = con.prepareStatement("SELECT lastVoteHopzone FROM vote_manager WHERE obj_Id="+player.getObjectId());
			
			ResultSet rset = statement.executeQuery();
			LOGGER.warn("[Vote Manager] Result " + rset.getLong("lastVoteHopzone"));
			
			if(rset.next())
			{
				LOGGER.warn("[Vote Manager] Data for xNewbic is readable. Continue!");
			...
			{
				LOGGER.warn("[Vote Manager] Failed to read xNewbic's data. Continue inserting!");
				PreparedStatement statement1 = con.prepareStatement("INSERT INTO vote_manager (obj_id,charName,lastVoteHopzone,lastVoteTopzone,hasVotedHop,hasVotedTop,tries,totalVotes) VALUES ("+player.getObjectId()+",'"+player.getName()+"',0,0,0,0,3,0)");

				
				statement1.execute();
				LOGGER.warn("[Vote Manager] I tried to insert data. Now I will see if was succesfully");
				
				PreparedStatement statement2 = con.prepareStatement("SELECT lastVoteHopzone FROM vote_manager WHERE obj_Id="+player.getObjectId());
				...
				if(rset1.next())
				{
					while (rset1.next())
					{
						LOGGER.warn("[Vote Manager] Variables worked just fine!");
						hopCdMs = rset1.getLong("lastVoteHopzone");
					}
				}
				else
				{
					LOGGER.warn("[Vote Manager] There must be a problem of reading data! Not working!");
				}
			}
		}
		catch (Exception e)
	...
	public static String topCd(L2PcInstance player)
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			statement = con.prepareStatement("SELECT lastVoteTopzone FROM vote_manager WHERE obj_Id=?");
		...
		if(topCdMs==0 || topCdMs<System.currentTimeMillis())
		{
			resultdate = "NOW";
		}
		
		else if(topCdMs>System.currentTimeMillis())
		{
			
			Date resultdateset = new Date(topCdMs + voteDelay);
			resultdate = sdf.format(resultdateset);
		}
	...
	public static void hopvote(final L2PcInstance player)
	{
		...
		class hopvotetask implements Runnable
		{
			...
			public void run()
			{
				if(Problemonhop == true)
				{
					...
					Problemonhop = false;
					System.out.println("I have error on topzone but i will give the reward... Please check me.");
				}
				else
				{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			statement = con.prepareStatement("SELECT lastVoteHopzone FROM vote_manager WHERE obj_Id=?");
			statement.setInt(1, player.getObjectId());
	...
	public static void hasVotedHop(L2PcInstance player)
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			PreparedStatement statement = con.prepareStatement("SELECT hasVotedHop FROM vote_manager WHERE obj_Id=?");
			statement.setInt(1, player.getObjectId());
	...
	public static void hasVotedTop(L2PcInstance player)
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			PreparedStatement statement = con.prepareStatement("SELECT hasVotedTop FROM vote_manager WHERE obj_Id=?");
			statement.setInt(1, player.getObjectId());
	...
	public static void updateVotes(L2PcInstance activeChar)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			PreparedStatement statement = con.prepareStatement("UPDATE vote_manager SET totalVotes=? WHERE obj_Id=?");
	...
	public static void setHasVotedHop(L2PcInstance activeChar)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			PreparedStatement statement = con.prepareStatement("UPDATE vote_manager SET hasVotedHop=? WHERE obj_Id=?");
	...
	public static void setHasVotedTop(L2PcInstance activeChar)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			PreparedStatement statement = con.prepareStatement("UPDATE vote_manager SET hasVotedTop=? WHERE obj_Id=?");
			statement.setInt(1, 1);
			statement.setInt(2, activeChar.getObjectId());
			statement.execute();
			statement.close();
	...
	public static int getTries(L2PcInstance player)
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			PreparedStatement statement = con.prepareStatement("SELECT tries FROM vote_manager WHERE obj_Id=?");
		 statement.setInt(1, player.getObjectId());
	...
	public static void setTries(L2PcInstance player, int tries)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			PreparedStatement statement = con.prepareStatement("UPDATE vote_manager SET tries=? WHERE obj_Id=?");
	...
	public static int getTotalVotes(L2PcInstance player)
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			PreparedStatement statement = con.prepareStatement("SELECT totalVotes FROM vote_manager WHERE obj_Id=?");
			statement.setInt(1, player.getObjectId());
			ResultSet rset = statement.executeQuery();
			while (rset.next())
			{	
	...
	public static int getBigTotalVotes(L2PcInstance player)
	{
		...
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			PreparedStatement statement = con.prepareStatement("SELECT SUM(totalVotes) FROM vote_manager");
			for (ResultSet rset = statement.executeQuery(); rset.next();)
			{
	...
	public static void updateLastVoteHopzone(L2PcInstance player)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			PreparedStatement statement = con.prepareStatement("UPDATE vote_manager SET lastVoteHopzone=? WHERE obj_Id=?");
	...
	public static void updateLastVoteTopzone(L2PcInstance player)
	{
		try (Connection con = L2DatabaseFactory.getInstance().getConnection())
		{
			PreparedStatement statement = con.prepareStatement("UPDATE vote_manager SET lastVoteTopzone=? WHERE obj_Id=?");
			statement.setLong(1, System.currentTimeMillis());
			statement.setInt(2, player.getObjectId());
			statement.execute();
		}
