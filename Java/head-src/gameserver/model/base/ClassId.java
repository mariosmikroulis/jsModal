...
public enum ClassId
{
	...
	public static ClassId getClassId(int raceId, boolean isMage)
	{
		try
		{
			for (ClassId cId : ClassId.values()) {
				if (cId.getRace() == null)
					continue;
				if (cId.isMage() == isMage && cId.getRace().ordinal() == raceId)
					return cId;
			}
		}
		catch (Exception e)
	...
	public static ClassId getClassId(String name) {
		boolean withSpaces = name.contains(" ");
		if (withSpaces) {
			for (ClassId cId : ClassId.values())
			...
			for (ClassId cId : ClassId.values())
				if (cId.name().replaceAll(" ", "").equalsIgnoreCase(name))
					return cId;
		}
		return null;
	}
