...
public class L2MerchantInstance extends L2FolkInstance
{
	...
	public void onBypassFeedback(final L2PcInstance player, final String command)
	{
		...
		{
			if(Config.TRADE_DROP_PICK_BUY_SELL_ENABLED && Config.SELL_TIME_DISABLE>0 && player.getOnlineTime()>Config.SELL_TIME_DISABLE)
			{
				showSellWindow(player);
			}
			else
			{
				player.sendMessage("You are in Sell Protection Mode for the next " + Config.SELL_TIME_TEXT + " since you have created this char. We apologise for that but it is kind of our own protection. If you believe that you are longer than that time, please restart your character now!");
				return;
			}
