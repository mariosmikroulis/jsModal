		...
		try
		{
			if ((reply[3] & 0x4) == 4)
			{
				if (ProtectConfig.ANNOUNCE_HACK_ATTEMPT && cl.getActiveChar() != null)
					Announcements.getInstance().announceToAll("Player " + cl.getActiveChar().getName() + " tried to use an extended program!");
				if (ProtectConfig.ON_HACK_ATTEMPT.startsWith("jail") && cl.getActiveChar() != null)
				{
					Announcements.getInstance().announceToAll("A current player has been going to prison.");
