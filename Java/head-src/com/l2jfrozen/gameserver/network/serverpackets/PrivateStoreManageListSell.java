...
public class PrivateStoreManageListSell extends L2GameServerPacket
{
	...
	{
		_activeChar = player;
		TradeItem[] _SellListNew = null;
		boolean _packageSaleNew = false;
		TradeItem[] _itemListNew = null;
		
		
		if(Config.TRADE_DROP_PICK_BUY_SELL_ENABLED && Config.SELL_TIME_DISABLE!=0 && Config.SELL_TIME_DISABLE>_activeChar.getOnlineTime())
		{
			_SellListNew=null;
		...
		else
		{
			...
			_packageSaleNew = _activeChar.getSellList().isPackaged();
			_itemListNew = _activeChar.getInventory().getAvailableItems(_activeChar.getSellList());
			_SellListNew = _activeChar.getSellList().getItems();
		}
		
		_packageSale = _packageSaleNew;
		_itemList = _itemListNew;
		_sellList = _SellListNew;
	}
