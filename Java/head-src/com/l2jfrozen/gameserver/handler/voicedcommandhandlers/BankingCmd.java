	...
	public boolean useVoicedCommand(final String command, final L2PcInstance activeChar, final String target)
	{
		...
		else if (command.equalsIgnoreCase("deposit"))
		{
			if (activeChar.getInventory().getInventoryItemCount(57, 0) >= Config.BANKING_SYSTEM_ADENA)
			{
				...
				activeChar.sendMessage("Thank you, now you have " + Config.BANKING_SYSTEM_GOLDBARS + " L2S Demon Coin, and " + Config.BANKING_SYSTEM_ADENA + " less adena.");
			}
			else
			{
		...
		else if (command.equalsIgnoreCase("withdraw"))
		{
			...
			if (activeChar.getInventory().getInventoryItemCount(3470, 0) >= Config.BANKING_SYSTEM_GOLDBARS)
			{
				...
				activeChar.getInventory().updateDatabase();
				activeChar.sendPacket(new ItemList(activeChar, true));
				activeChar.sendMessage("Thank you, now you have " + Config.BANKING_SYSTEM_ADENA + " Adena, and " + Config.BANKING_SYSTEM_GOLDBARS + " less L2S Demon Coin(s).");
			}
			else
			{
				activeChar.sendMessage("You do not have any L2S Demon Coin to turn into " + Config.BANKING_SYSTEM_ADENA + " Adena.");
			}
		}
		return true;
	}
