...
public final class L2TeleporterInstance extends L2FolkInstance
{
	...
	public void onBypassFeedback(final L2PcInstance player, final String command)
	{
		...
		else if (command.startsWith("GrandBoss_"))
		{
			if (command.startsWith("GrandBoss_Valakas"))
			{
				doPartyTeleport(player, 67239);
			}
			else if (command.startsWith("GrandBoss_Antharas"))
			{
				doPartyTeleport(player, 67240);
			}
			else if (command.startsWith("GrandBoss_Baium"))
			{
				doPartyTeleport(player, 67241);
			}
			else if (command.startsWith("GrandBoss_Zaken"))
			{
				doPartyTeleport(player, 67242);
			}
			else if (command.startsWith("GrandBoss_QueenAnt"))
			{
				doPartyTeleport(player, 67243);
			}
			else if (command.startsWith("GrandBoss_Frintezza"))
			{
				doPartyTeleport(player, 67244);
			}
			else if (command.startsWith("GrandBoss_Core"))
			{
				doPartyTeleport(player, 67245);
			}
			else if (command.startsWith("GrandBoss_Orfen"))
			{
				doPartyTeleport(player, 67246);
			}
		}
	...
	public static void doPartyTeleport(L2PcInstance player, int val)
	{
		if (player.getParty() == null)
		{
			player.sendMessage("Your party must have more than 5 members to enter this zone.");
			return;
		}
		
		if (player.getObjectId() != player.getParty().getPartyLeaderOID())
		{
			player.sendMessage("Only the party leader can teleport.");
			return;
		}

		if(player.getParty().getMemberCount() < Config.TELEPORT_PARTY_MIN_RB)
		{
			player.sendMessage("Your party must have more than "+ Config.TELEPORT_PARTY_MIN_RB +" members to enter this zone.");
			return;
		}
		
		final L2Party party = player.getParty();
		L2Object teleporter = player.getLastFolkNPC();
		for (L2PcInstance p : party.getPartyMembers())
		{		   
			if (!p.isInsideRadius(teleporter, 900, true, true))
			{
			   player.sendMessage("All party members must be near the gatekeeper.");
			   return;
			}
		}
